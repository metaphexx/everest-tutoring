    ...
    <AdminShell sub="Communications log">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="portal-lede">Every reminder, alert and message - sent or previewed.</p>
        </div>
      ...
      <div className="glass-card glass-card-pad mt-5">
        {notifications.length === 0 ? (
          ...
          <div className="space-y-1.5">
            {notifications.map((n) => (
              <div key={n.id} className="flex items-center gap-3 p-3 rounded-xl" style={{ background: 'rgba(255,255,255,.5)' }}>
                <span className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: 'rgba(0,157,255,.1)', color: '#009dff' }}>
                  {n.channel === 'email' ? <Mail size={16} /> : <MessageSquare size={16} />}
                </span>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-dark truncate">{n.subject ?? n.body}</p>
                  <p className="text-xs text-slate-500 truncate">{n.body}</p>
                </div>
                <div className="text-right flex-shrink-0">
                  <Badge size="sm" className={STATUS_STYLE[n.status] ?? 'bg-slate-100 text-slate-600'}>{n.status}</Badge>
                  <p className="text-[11px] text-slate-400 mt-1">{n.type} · {format(new Date(n.createdAt), 'd MMM HH:mm')}</p>
                </div>
              </div>
            ))}
          </div>
